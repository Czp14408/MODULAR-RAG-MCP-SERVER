...
| B8 | Vision LLM 抽象接口与工厂集成 | [x] | 2026-03-01 | 自动验收通过；已完成 BaseVisionLLM 抽象、LLMFactory.create_vision_llm 与工厂路由测试 |
| B9 | Azure Vision LLM 实现 | [x] | 2026-03-01 | 自动验收通过；已完成 AzureVisionLLM（路径/base64输入、压缩扩展点、错误处理）与 mock HTTP 测试 |
...
| C11 | BM25Indexer（倒排索引+IDF计算） | [x] | 2026-03-04 | 自动验收通过；已完成 BM25 索引构建/持久化/查询、IDF 公式校验与 roundtrip 单测 |
| C12 | VectorUpserter（幂等upsert） | [x] | 2026-03-04 | 自动验收通过；已完成稳定 chunk_id 生成、向量库 upsert 适配与幂等性单测 |
| C13 | ImageStorage（图片存储+SQLite索引） | [x] | 2026-03-04 | 自动验收通过；已完成图片落盘、SQLite WAL 索引映射与 collection 查询单测 |
| C14 | Pipeline 编排（MVP 串起来） | [x] | 2026-03-06 | 自动验收通过；已完成 integrity→load→split→transform→embed→upsert 串行编排、图片索引入库与集成测试 |
| C15 | 脚本入口 ingest.py | [x] | 2026-03-06 | 自动验收通过；已完成 CLI 参数解析、离线 ingest 执行、首次成功二次跳过的 e2e 验证 |
...
| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| G1 | Dashboard 基础架构与系统总览页 | [x] | 2026-03-08 | 自动验收通过；已完成 Streamlit 多页面导航、Overview 页面、上下文构建与 start_dashboard.py 启动脚本 |
| G2 | DocumentManager 实现 | [x] | 2026-03-08 | 自动验收通过；已完成跨 Chroma/BM25/ImageStorage/FileIntegrity 的 list/detail/delete/stats 协调与单测 |
| G3 | 数据浏览器页面 | [x] | 2026-03-08 | 自动验收通过；已完成集合筛选、文档详情、chunk 预览与图片元数据浏览 |
| G4 | Ingestion 管理页面 | [x] | 2026-03-08 | 自动验收通过；已完成文件上传触发 pipeline、进度条展示与文档删除操作 |
| G5 | Ingestion 追踪页面 | [x] | 2026-03-08 | 自动验收通过；已完成 ingestion trace 列表与阶段耗时柱状视图 |
| G6 | Query 追踪页面 | [x] | 2026-03-08 | 自动验收通过；已完成 query 关键字过滤、query trace 列表与阶段对比视图 |

#### 阶段 H：评估体系

| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| H1 | RagasEvaluator 实现 | [x] | 2026-03-08 | 自动验收通过；已完成 ragas 依赖检查、mock 后端评估与缺依赖可读错误测试 |
| H2 | CompositeEvaluator 实现 | [x] | 2026-03-08 | 自动验收通过；已完成多评估器指标合并与组合评估单测 |
| H3 | EvalRunner + Golden Test Set | [x] | 2026-03-08 | 自动验收通过；已完成 golden test set 读取、HybridSearch 评估运行、evaluate.py 脚本与指标汇总 |
| H4 | 评估面板页面 | [x] | 2026-03-08 | 自动验收通过；已完成 Dashboard 评估页 backend 选择、运行按钮与评估报告展示 |
| H5 | Recall 回归测试（E2E） | [x] | 2026-03-08 | 自动验收通过；已完成 recall e2e 测试，验证 hit_rate/mrr 达到阈值 |
